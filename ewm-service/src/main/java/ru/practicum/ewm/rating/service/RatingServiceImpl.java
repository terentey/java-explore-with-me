...
import ru.practicum.ewm.EwmClient;
...
public class RatingServiceImpl implements RatingService {
    ...
    public List<RatingDto> findAllRatingByUserId(long userId, LocalDateTime rangeStart, LocalDateTime rangeEnd) {
        ...
        try {
            List<RatingStatsDto> ratings = objectMapper.readValue(
                    client.rating(rangeStart, rangeEnd, null, String.format("/users/%d", userId)).getBody(),
                    new TypeReference<>(){});
            return RatingMapper.mapToRatingDto(ratings);
    ...
    public List<RatingDto> findAllRatingByUserIdAndEventId(long userId, long eventId, LocalDateTime rangeStart, LocalDateTime rangeEnd) {
        ...
        try {
            List<RatingStatsDto> ratings = objectMapper.readValue(
                    client.rating(rangeStart, rangeEnd, uris, null).getBody(),
                    new TypeReference<>(){});
            return RatingMapper.mapToRatingDto(ratings);
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
